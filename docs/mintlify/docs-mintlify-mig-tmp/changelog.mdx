...
- **tag filter across screen, audio, and memories** — `/search` and the MCP `search-content` tool now accept a comma-separated tag filter that AND-matches across screen captures, audio, and memories in one namespace. Namespaced tags (`person:`, `project:`, `topic:`) link captures to facts, so a single query like `person:alice,project:atlas` pulls everything tagged with both. Index-driven and fast even on large libraries. See [search screen history](/search-screen-history).
- **native drag-to-grant permission flow** — granting screen, accessibility, and input monitoring on macOS now opens a small drag panel you can drop screenpipe's icon into the right system-settings list with, instead of hunting for the toggle. The panel auto-hides when screenpipe is focused, detects manual toggles in the background, and closes itself the moment the permission flips. See [permissions](/permissions).
- **HD timeline playback at native framerate** — HD recordings (10/30 fps) now play back at their real framerate in the timeline instead of stepping ~1 fps per second. The master clock still drives audio + slider, and playback re-syncs if video drifts more than ~0.75s, so motion is smooth and remains aligned. Non-HD chunks are unchanged.
- **connection chips in chat with one-click "Try in Chat"** — connection cards in **settings → connections** now have a "Try in Chat" action that drops a prefilled chip into the composer with the right integration prefix, so you can ask the agent to use a specific connection without typing it out. See [connections](/connections).
- **redesigned connections page** — **settings → connections** is reorganized into categorized groups (productivity, meetings, dev tools, …) for faster scanning across the now-large connection catalog. See [connections](/connections).
- **searchable settings** — a fuzzy search bar at the top of **settings** matches across every panel and sub-field, so you can jump straight to the setting you want and the page scrolls it into view.
- **Ask source citations open their captures** — answers in the chat list their sources, and screen / audio / meeting sources are now clickable: clicking one jumps the timeline to the exact captured moment behind the answer. Web, file, memory, db, and connector sources are unchanged.
- **Mochi flashcards connection** — connect mochi.cards via API key under **settings → connections** to let the agent read and create spaced-repetition flashcards. The key is held in the local credential store and never exposed to pipes or the LLM. See [connections](/connections).
- **Plaud connection (one-click OAuth)** — a new Plaud card in **settings → connections** connects the Plaud remote MCP server (meetings + recordings) via OAuth, with no manual setup. See [connections](/connections).
- **font size setting for app-wide text scaling** — a new font-size control in **settings → display** scales the whole app UI up or down to taste.
- **Getting started tutorial card in Help** — the **Help** section now features the full screenpipe tutorial video as a card at the top, so new users have an obvious place to start.
- **chat history view refresh** — the chat history page has a refreshed layout with a sticky header for tabs and search that stays put as you scroll.
- **screenpipe-api and screenpipe-cli skills in agent cards** — the Hermes and OpenClaw skill tabs now offer both the API and CLI screenpipe skills behind a single switcher, and the served `SKILL.md` is generated from the canonical source so the copy never drifts.
- **HD start confirmation** — clicking "+ HD" on a meeting notification now posts an in-app confirmation that HD capture has started, instead of being silent. See [meeting intelligence](/meeting-intelligence).

### updates

- **pipe fallback presets actually run on errors and timeouts** — a pipe declared with `preset: ["primary", "fallback"]` used to only fall back when the primary's circuit breaker had already tripped, so timeouts and executor crashes never advanced to the fallback. The retry path now advances past any failed preset on any failure (bounded by a max depth), so fallbacks reliably kick in. See [pipes](/pipes).
- **smaller, lighter local PII redaction model** — the bundled text-PII model is replaced with a vocab-pruned build that drops from 266 MB to 149 MB on disk, with proportionally lower memory use in the redact worker. Detection spans on names, emails, phones, SSNs, and persons are unchanged. See [privacy filter](/privacy-filter).
- **`--ignored-urls` actually filters captures** — the CLI flag was parsed but never reached the capture layer, so URL-based privacy filtering silently did nothing. Blocked URLs are now dropped from screen, accessibility, and HD capture across macOS and Windows. See the [CLI reference](/cli-reference) and [privacy filter](/privacy-filter).
- **mDNS LAN device discovery is now opt-in** — multi-device sync's mDNS browse/advertise used to open a multicast socket at every startup, which triggered macOS's "Local Network" prompt even for users who never used multi-device. mDNS is now off by default; enable it with `screenpipe --enable-mdns` or `SCREENPIPE_ENABLE_MDNS=1`. SSH and Tailscale discovery are unaffected. See the [CLI reference](/cli-reference).
- **lower idle and active CPU on Windows** — the Windows CLI and UI recorder both wake up less often when idle, and the Windows capture path starts faster and uses less CPU under active load.
- **plan labels match the pricing page** — Basic ($25) and lifetime users were being shown "Free plan" in account settings and the tray because the label only checked whether cloud was active. Plans are now labelled by their actual subscription (Basic, Business, Enterprise) everywhere. Remaining "Pro" copy in the upsell, sync card, calendar gates, and preset toasts is renamed to "Business".
- **clearer data-deletion settings** — **settings → privacy** now splits the one-time "erase recent activity" action from the ongoing storage policy into two cards, keeps the "older than N days" cutoff visible at all times, shows the current setting inline, and renames the ambiguous "video + audio only" option to "drop video + audio, keep text". See [privacy data flow](/privacy-data-flow).
- **clearer pipe schedule day toggles** — the day-of-week toggles in pipe schedules now use full day labels and an "enable / disable" affordance instead of unlabeled on/off dots. See [pipes](/pipes).
- **actionable "model unavailable" errors from the AI gateway** — a 404 / 400 from a provider used to surface as a generic "request failed (404), try again" message, so users retried the same model forever. The gateway now says the model isn't enabled on your account or key and to pick a different one; 401 / 403 say the key was rejected.
- **actionable model-download errors on corporate networks** — pyannote model downloads on networks with TLS interception, proxies, or captive portals used to fail with a raw "error sending request". The error now names the likely cause (corp root cert, DNS, firewall, proxy auth, captive portal) and what to try.
- **"Book a call" row removed from settings → feedback** — the 1:1 founder call link is gone from individual feedback. Teams can still book via **settings → team**, and the engine-startup and "send logs" support paths are unchanged.

### bug fixes

- **no more ghost keystrokes on macOS recorder shutdown** — quitting the recorder (e.g. from the tray) could inject phantom characters into the focused app — typed text coming back mangled or duplicated, most visibly in Chromium / Electron apps like Discord. The macOS permission probe that caused it is gone, the capture tap now drops events the instant teardown starts, and the tap teardown is idempotent.
- **cursor placement and text selection work mid-sentence again** — fixed a regression that broke clicking to place the caret or selecting text in the middle of a sentence in app text fields.
- **dark theme paints native scrollbars dark on Windows** — picking the dark theme while the OS was in light mode (common on Windows) left the WebView painting light scrollbars and form controls in an otherwise dark UI. Native controls now match the app theme regardless of the OS setting.
- **Windows OCR honors your language selection** — fixed a bug where the Windows OCR engine ignored the language picked under **settings → OCR** and fell back to the system default.
- **timeline arrow-right keeps working while recording** — during active capture, the timeline could fall out of sync with the live edge and stop advancing after a few right-arrow presses. Navigation now stays correct across live prepends and respects the active monitor / app filter. See [search screen history](/search-screen-history).
- **live transcript segments are never dropped on a coverage miss** — long meetings could appear to "stop transcribing" mid-call because live segments whose nearest same-device audio chunk fell outside a coverage window were silently discarded before post-call surfaces (notes, timeline, search) read them. The mirror now falls back to the nearest same-device chunk rather than dropping the segment, keeping the real timestamp. See [meeting intelligence](/meeting-intelligence).
- **no more cross-device echo doubling in live transcripts** — without headphones the mic picks up the speaker output, so the remote's speech was transcribed on both the mic and the system-audio live streams and appeared twice in the live transcript. Input segments that are mostly covered by a nearby output segment are now suppressed; your own speech is preserved. See [meeting intelligence](/meeting-intelligence).
- **"Coming up" calendar panel handles multiple Google accounts** — with more than one Google account connected, the panel queried calendar events without specifying which account, the backend returned 401, and the panel showed a generic "calendar needs attention" card even when every account was healthy. It now enumerates and merges every connected account and only flags an error when all of them fail. See [meeting intelligence](/meeting-intelligence).
- **background pipe pages no longer hijack the chat browser via `/eval`** — the owned-browser sidebar was supposed to ignore navigations owned by other chats or pipes, but the navigate-and-scrape `/eval` path dropped the owner tag, so a background pipe's page kept popping into whatever chat was on screen. The owner is now threaded through `/eval` too. See [pipes](/pipes).
- **owned-browser title changes from `/eval` are preserved** — `document.title` updates made through owned-browser `/eval` could be reverted to the pre-eval title; the post-eval title is now what's reported and kept.
- **meeting note header pills line up** — the clock pill rendered ~2 px taller than the date and attendees pills, leaving the header row uneven. All three pills now have a fixed height. See [meeting intelligence](/meeting-intelligence).
