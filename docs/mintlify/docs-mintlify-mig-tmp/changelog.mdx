...
- **Readwise connection** — connect your Readwise account from **settings → connections** so highlights and notes join the rest of your timeline and are available to pipes and chat. see [connections](/connections).
- **Workflowy connection** — append bullets to a Workflowy outline from pipes and chat; configure with an API key in **settings → connections**. see [connections](/connections).
- **VS Code integrated terminal is now captured** — terminal output (and the terminal session name as the active window title) is now included in accessibility capture for VS Code, so what you ran and saw in the terminal feeds AI context.
- **Search meeting notes by title, attendees, or note body** — the meeting notes list now has a search box with a debounced server-side `q` filter on `GET /meetings`, plus an explicit "show more" instead of silent truncation. see [meeting transcription](/meeting-transcription) and the [API docs](/for-developers).
- **`Cmd+N` / `Ctrl+N` opens a new chat** — works from the home page, mirroring the "New chat" sidebar button.
- **`Ctrl+Tab` switches between recent chats** — quick keyboard switch across your most recent chat sessions, similar to tab cycling in a browser.
- **Gemini 3.5 Flash in chat** — added to the model catalog and promoted to the top of the "auto" waterfall for both text and vision, with the existing open-weight Vertex chain kept as a cheaper fallback.
- **Per-stream enterprise sync controls** — admins can now choose, per organization, which streams (frames, audio, UI events, memories, snapshots) cross the wire instead of one all-or-nothing toggle. Existing deployments default to all-on. see [teams](/teams).

### updates

- **Streamlined onboarding** — the 3-way "what brings you here?" path picker is replaced by a single default-checked bundle of recommended pipes with a "customize" expander, the encrypt-data step has been removed (the encrypted store is on by default), and the pipe step is narrowed to digital-clone + personal-crm. fewer clicks to a working setup. see [getting started](/getting-started).
- **Live chat panel removed from feedback** — the in-app "Live chat with us" panel has been removed; use email support or Discord instead. see [troubleshooting](/troubleshooting).
- **Clearer Gmail connection errors** — the chat panel now surfaces the real Google API status (e.g. `401 invalid_credentials`, quota errors) when a Gmail call fails, instead of a generic 500. see [connections](/connections).
- **Source citations in any agentic chat** — chats that take multiple assistant turns to answer (not just pipe-runs) now show a single aggregated source-citation footer at the end, including in markdown exports.

### bug fixes

- **Recording no longer stuck on "Starting"** — fixed a case where stale monitor IDs would leave the engine wedged in the "Starting" state on launch; recovery is automatic.
- **System audio captured when following system defaults (macOS)** — the desktop app now resolves the `default` audio sentinel the same way the CLI does, so new users on macOS no longer end up with mic-only recordings.
- **Process-tap sample rate on aggregate devices (macOS)** — system audio capture now reads the aggregate device's nominal sample rate, fixing distorted or empty audio on setups that route through an aggregate.
- **Audio default enrollment and recovery** — hardened the default-input enrollment path and liveness checks so flapping devices recover cleanly without manual restart.
- **No more bogus "FullPause" on macOS Low Power Mode** — unplugging at 100% battery no longer silently kills capture. macOS Low Power Mode is now treated as a throttling hint (drop to Saver), not a stop signal; only `≤10%` battery triggers a real FullPause.
- **No crash on low battery** — fixed a profile-info gap that could crash the engine when crossing the low-battery threshold.
- **"Whisper" removed from low-battery notification** — the notification copy is now accurate to what the app actually does.
- **No more phantom "meeting detected" toast on cal.com / Instacart** — tightened the meeting detector so dashboards with meeting-shaped UI no longer fire the "join and take notes" toast. the misleading "join and take notes" button has also been dropped from the in-progress meeting toast. see [meeting transcription](/meeting-transcription).
- **Excluded apps are properly filtered on Windows** — apps you add to capture exclusions are now actually skipped during accessibility capture on Windows. see [privacy filter](/privacy-filter).
- **Multi-account Google Docs and Sheets** — connecting more than one Google account now lists each instance separately, forces the Google/Microsoft account picker on connect, and routes the Google Docs proxy to the right instance. see [connections](/connections).
- **"Not connected" vs multi-account ambiguity** — the connections panel now distinguishes a missing connection from a connection that can't pick an account, with the right call-to-action for each. see [connections](/connections).
- **Chat sidebar stays in sync across windows** — conversations saved in another window now appear in your sidebar without a refresh, and switching chats no longer overwrites a newly-opened one with a stale snapshot.
- **Chat preset stays put** — switching the model dropdown or sending a message no longer snaps the chat preset back to the default.
- **Mid-tool-call assistant replies stay in one bubble** — switching away while a tool call is running no longer splits the reply into two bubbles when you come back. pipe-run transcripts also coalesce assistant turns and show real work duration.
- **Assistant message actions hidden while streaming** — copy / regenerate / edit buttons no longer appear (and accidentally fire) before the reply is complete.
- **Branched chat sidebar / session sync** — branching a chat now shows up correctly in the sidebar and resolves to the right session.
- **Local media attachments render reliably in chat** — fixed a case where image and video attachments could fail to display.
- **First-launch boot crash on the desktop app** — fixed a React #185 crash that could hit on the very first launch.
- **Intel Mac Pi install** — fixed `Bad CPU type in executable` when installing the Pi agent on Intel Macs.
- **HTTP MCP sessions register after `initialize`** — fixed an order-of-operations bug that could drop HTTP MCP sessions on connect. see [MCP server](/mcp-server).
- **Scheduled pipes get the right local API key** — `SCREENPIPE_LOCAL_API_KEY` is now exported to scheduled pipe subprocesses and used consistently across pipes, fixing 401s in cron-triggered runs. see [pipes](/pipes).
- **Cron pipes don't fire on install or for stale schedules** — newly-installed pipes (or pipes whose last-run timestamp is in the past) no longer execute immediately; the next scheduled tick is honored. see [pipes](/pipes).
- **`/search` accepts flexible booleans** — query params now accept `1`/`0`/`yes`/`no`/empty in addition to `true`/`false`. see the [API docs](/for-developers).
- **Windows updates stop sidecars first** — the NSIS installer now shuts down bundled sidecars before applying the update, eliminating "file in use" failures during auto-update.
- **Desktop login URL fixed** — the in-app login link now points at the right endpoint.
- **Overlay shortcut pill button clicks restored** — buttons inside the expanded shortcut pill are clickable again.
- **Power-mode selector stays visible** — Settings → Power keeps the selector mounted even when the engine `/power` endpoint is briefly unreachable.
- **Build reliability** — `downloadFile` retries are tighter and sidecar verify timeouts no longer fail clean installs on slow networks.
