...
- **Odoo connection** — first-class Odoo ERP integration in **settings → connections**: enter URL, login, and API key, and the AI can query and update leads, helpdesk tickets, sales orders, and contacts over JSON-RPC. database is auto-detected for `*.odoo.com`. see [connections](/connections) and the [connection reference](/connection-reference).
- **Krisp MCP connection** — a dedicated Krisp card in **settings → connections** wires Krisp's meeting transcripts, notes, and action items into screenpipe with one-click OAuth, instead of pasting URLs into the custom MCP form. see [connections](/connections).
- **multi-vault Obsidian** — the Obsidian connection now manages multiple vaults from one panel: auto-discover via `obsidian.json`, add more manually, and disconnect any of them. each vault surfaces to chat and pipes as its own labeled block. see [Obsidian](/obsidian) and [connections](/connections).
- **keyboard and clipboard capture toggles** — Settings → Privacy now has explicit toggles for keyboard and clipboard capture, and both default to **off** on new installs. on-screen text (OCR and accessibility tree) is unaffected, so Rewind and Ask still work the same; the raw keystroke and clipboard streams are simply opt-in. see [privacy filter](/privacy-filter).
- **redact IDs, URLs, and dates** — the "fields to redact" selector under **settings → privacy → AI PII removal** now exposes IDs (SSNs, credit cards, account numbers), URLs, and dates as opt-in categories, in addition to the existing fields. secrets stay always-on. see [privacy filter](/privacy-filter).
- **enterprise PII policy applied on devices** — workspace admins can now force the PII backend (local vs Tinfoil cloud) and which categories to redact from the dashboard, and devices honor it on the next policy poll. locked settings are greyed out in the in-app privacy section so they can't be overridden locally. see [teams](/teams) and [privacy filter](/privacy-filter).
- **`total_active_minutes` on activity summary** — `/activity-summary` now returns a deterministic, uncapped grand total of active minutes for the whole range, alongside the existing per-app and per-window breakdowns, so you don't have to sum a top-N list (and get a different number each run). see [API recipes](/api-recipes).
...
- **better meeting detection** — Zoom calls that start already in screen-share are now detected from the floating control bar, and any in-progress non-all-day calendar event keeps a detected meeting alive while controls vanish (e.g. during share or minimize). see [meeting intelligence](/meeting-intelligence).
- **live transcripts appear on the timeline** — live meeting finals are now mirrored into the regular transcript store and surface on the timeline and in search, with global cross-meeting speaker IDs (instead of just per-stream "Speaker 2" labels). see [meeting transcription](/meeting-transcription) and [meeting intelligence](/meeting-intelligence).
- **"disable timeline" moved to Display settings** — the toggle now lives in **settings → display** next to Timeline Mode. when it's on, the Timeline item disappears from the sidebar, the shortcut and reminder overlay no-op, and timeline-only background work is fully released. see [search screen history](/search-screen-history).
- **calendar picker on meeting notes opens connections** — when no calendar is connected yet, opening a calendar from a meeting note now jumps straight to **settings → connections** instead of a dead picker. see [connections](/connections).
- **owned-browser cookie consent is a tri-state preference** — the embedded browser now asks once whether to use your browser cookies for the current site, then persists "enabled" or "disabled" so it never re-prompts. "use browser login" is a clean checkable toggle in the browser menu, with a "retry page" shortcut. on macOS, screenpipe preflights Keychain access so granted users only see a warning when the OS would actually prompt. see [connections](/connections).
...
- **no more phantom or doubled text in Electron and Chromium apps** — fixed a regression where screenpipe's accessibility walker re-poked Chromium every 60s, which could commit a pending IME or autocomplete buffer into the focused field. screenpipe now sets up enhanced accessibility once per app and stops re-poking it.
- **owned browser stops popping over your view** — pipes and background agents driving the embedded browser no longer flash it over your meeting notes, timeline, or settings on every snapshot, eval, or navigate. background navigations also stay scoped to the chat that issued them, so a Reddit pipe doesn't bleed its page into whatever chat you opened next.
- **image PII redaction actually runs again** — the on-device image PII model is back to detecting PII reliably on real screen frames; the previous version under-detected and the one before that silently errored on every frame after a model bump. see [privacy filter](/privacy-filter).
- **CLI flags can re-enable a paused engine** — passing `--audio-device`, `--use-system-default-audio`, `--monitor-id`, or `--use-all-monitors` now overrides a persisted `disable_audio`/`disable_vision` setting, instead of being silently ignored. an explicit `--disable-audio` or `--disable-vision` on the same command still wins. see the [CLI reference](/cli-reference).
- **meetings keep your audience in the recording** — fixed a transcript-merge bug that suppressed background-recovered turns from other participants when the user was talking densely, leaving meeting notes "70% me, nothing from my audience". input and output captures are now deduped independently. see [meeting transcription](/meeting-transcription).
- **meetings survive a brief lull in output audio** — a deferred output-audio segment no longer ends an in-progress call. see [meeting intelligence](/meeting-intelligence).
- **only one open meeting at a time** — fixed a race that could leave two parallel "in progress" rows for the same call; the database now enforces a single open meeting and heals any pre-existing duplicates on update. see [meeting intelligence](/meeting-intelligence).
- **audio worker survives sweep panics** — a panic in the audio reconciliation sweep no longer kills the background worker silently; the loop catches it, logs, and keeps going, so pending chunks don't pile up until the next app restart.
- **two top crashes retired** — fresh installs and relocated data directories no longer fail with "unable to open database file"; the data directory is created on demand. chat requests with no model field no longer take down the AI gateway. see [for developers](/for-developers).
- **macOS OCR bounding boxes** — Apple OCR now returns bounding-box coordinates again, fixing missing highlights in screen-history results.
- **CRLF-authored pipes** — the pipe-notification bell now correctly reads and writes frontmatter in pipe.md files saved with Windows line endings, instead of showing the wrong toggle state or duplicating the frontmatter block. see [pipes](/pipes).
- **check-now toast is honest** — clicking "check for updates" while an update is already pending now says "update ready, restart to install" instead of "you're up to date".
- **onboarding respects "no pipes"** — skipping the pipe step with nothing selected no longer auto-installs the default bundle. see [getting started](/getting-started).
- **timeline Live Text overlay no longer eats clicks in chat** — the native macOS VisionKit overlay is hidden when the timeline window loses focus, so it can't sit on top of the chat input and silently swallow keystrokes.
- **Gemini stops web-searching your own data** — fixed a chat regression where Gemini Flash (the `auto` default) would reach for web search to look up the local screenpipe API after a 4xx, dumping raw Google grounding into the conversation and abandoning the task. web search is now scoped to genuinely external info and tool calls pass through to the agent loop as intended.
- **`rewind://` link clicks open the browser again** — schemeless URL pills and "top sites" links in Rewind now normalize to `https://` before opening, instead of silently failing. see [search screen history](/search-screen-history).
- **chat: no more stray highlight in empty areas** — click-dragging across the blank space in the chat welcome screen no longer paints a fake selection.
- **meeting notes: bottom of the editor is clickable** — the docked footer (controls + transcript panel) no longer overlaps the note editor, so you can click the last few lines instead of only reaching them with arrow keys. see [meeting intelligence](/meeting-intelligence).
- **Apple Calendar tile hidden off macOS** — the **settings → connections** page no longer shows the Apple Calendar tile on Windows and Linux, where it can't work. see [connections](/connections).
- **pipe preset IDs are trimmed on parse** — leading and trailing whitespace in preset IDs no longer causes lookup failures. see [pipes](/pipes).
